...
## Completion Summary
Extracted a reusable `Pagination` component + `usePagination` hook (mirroring the exact visual design of the existing trip-detail attractions pagination) and applied it to the Top Explorers table on `/analytics`, capped at 5 rows per page. Rank numbers and gold-highlight styling correctly account for the current page rather than resetting per page. No other tables in the app currently need pagination (the only other `<table>` is a small static opening-hours grid). Confirmed by user. Closed 2026-07-29.
