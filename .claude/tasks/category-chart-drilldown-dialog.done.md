...
## Completion Summary
Clicking a sub-category pie slice or legend row on `/profile` or `/analytics` now opens a `CategoryAttractionsModal` listing the matching attractions (owner-scoped on profile, site-wide on analytics), extending `GET /api/attractions` with a type-only query mode and an `ownerId` scoping param. Mid-flow, the user additionally requested that clicking a row in that dialog opens the attraction's full detail card, which was added by reusing the existing `AttractionDetailModal` unchanged. Confirmed by user 2026-07-25.
