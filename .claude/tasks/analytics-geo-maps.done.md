...
## Completion Summary
Built Leaflet-based geographic visualizations for the analytics drill-down panels: a world choropleth map for the Countries stat card (visited countries shaded by attraction count) and a city marker map for the Cities Covered stat card (circle size scaled by count, with a country filter dropdown to scope the view). Caught and fixed a property-name mismatch bug that had silently broken country matching (`ADMIN` vs `name` in the GeoJSON source), plus added a country-name alias table for known wording differences. User confirmed working. Closed 2026-06-30.
