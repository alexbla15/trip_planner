...
## Post-Deploy Fix
User reported "data lost" on Gardaland and the table "not reflecting all pricing data" after the first deploy. Investigated via a direct DB read and a raw `GET /api/attractions/:id` call — all 8 of Gardaland's price tiers were intact in the database and in the API response; nothing was actually lost. The real bug: the Prices table still paginated at `ATTRACTIONS_PAGE_SIZE` (5) per page (inherited from the pre-existing pagination feature, now applied on top of the new grouping), so an attraction with more than 5 tiers only showed its first 5 until "Next" was clicked — reading as missing/lost data, made worse once tiers are also split into day-type groups (a group could be cut off mid-page).
