...
| `openingHours` | `Record<"Mon"\|...\|"Sun", {closed: boolean, open: "HH:MM", close: "HH:MM"}>` (24h strings — see `NewAttractionModal.tsx`). **Never fill this with generic "typical hours for the venue type."** Look up each venue's real hours online (WebSearch/WebFetch) before writing this field. If a source gives varying hours per day (e.g. closed Monday, weekend hours differ), encode that per-day rather than flattening to one range. If, after a real search attempt, no source states the hours, **omit the `openingHours` field entirely for that record** (don't set it to anything, and don't add a doc with the key present but empty) rather than inventing one — call it out by name in your summary to the user instead of silently guessing. If a doc already exists with fabricated/generic hours and you can't verify them, `$unset` the field rather than leaving guessed data in place. |
...
2b. **Research each venue online before writing prices or opening hours** (WebSearch, then WebFetch the venue's own site or a listing site if search snippets are ambiguous). This is required, not optional — do it up front for the whole batch so the insert script only ever writes real values. Coordinates are the one field still allowed to be a best-effort approximation from general knowledge (flagged as such); price and openingHours are not.
...
- Don't fabricate precision you don't have — flag approximate coordinates as such in your final summary to the user instead of presenting them as verified facts. Price and opening hours are held to a stricter bar: they must come from an actual web search/fetch, not general knowledge or "typical for this venue type" reasoning. If a specific venue's hours/price genuinely can't be found online after a real attempt, leave the field unset and say so by name — don't fill in a plausible-looking placeholder.
